...
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
static uint32_t zero_x = 0;
static uint16_t tim1_value = 0;
...
{
  if (GPIOD->IDR & (uint8_t)GPIO_PIN_2) { // Rising edge only
...
INTERRUPT_HANDLER(TIM1_CAP_COM_IRQHandler, 12)
{
  ...
  */
  uint8_t sr = TIM1->SR1;
  if (sr & (uint8_t)TIM1_FLAG_CC1) {
    PWM_ON;
    TIM1->SR1 &= (uint8_t)(~TIM1_IT_CC1);
  }
  if (sr & (uint8_t)TIM1_FLAG_CC2) {
    PWM_OFF;
    TIM1->SR1 &= (uint8_t)(~TIM1_IT_CC2);
  }
  if (sr & (uint8_t)TIM1_FLAG_CC3) {
    PWM_ON;
    TIM1->SR1 &= (uint8_t)(~TIM1_IT_CC3);
  }
  if (sr & (uint8_t)TIM1_FLAG_CC4) {
    PWM_OFF;
    TIM1->SR1 &= (uint8_t)(~TIM1_IT_CC4);
  }
}
